...
# 6) Sponsor Dashboard (pass sponsor ID on the page)
...
def sponsor_dashboard(sponsor_id):
    ...
    return render_template('sponsor_dashboard.html', sponsor=sponsor, job_postings=job_postings)
...
def job_posting_details(sponsor_id, job_id):
    job_posting = db.jobPostings.find_one({"_id": job_id, "sponsor": sponsor_id})
    return render_template('job_posting_details.html', job_posting=job_posting)
...
# 9) Route for top candidates based on sponsor and job ID
@app.route('/sponsor/<sponsor_id>/job/<job_id>/top_candidates')
def top_candidates(sponsor_id, job_id):
    # Logic to find top candidates for the job posting
    # For example, you can filter based on skills or experience
    top_candidates = db.candidates.find({"skills": {"$in": db.jobPostings.find_one({"_id": job_id})['requiredSkills']}})
    return jsonify([candidate for candidate in top_candidates]), 200
